...
- 開催開始日時 (`started_at`) が実行時刻以前のイベントは投稿しない。connpass API の返却対象が突然広がって過去イベントが「新着 ID」として大量に見える場合の防御。`started_at` が null のものは投稿対象
- 1 回の run で新着イベントが 20 件を超えた場合は異常(connpass 側の返却ウィンドウ拡大または `posted-events.json` の state 喪失)とみなし、1 件も投稿せずに run を失敗させる。復旧は `posted-events.json` に手動で ID を追記して行う
